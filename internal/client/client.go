...
func New(cfg *config.Config, timeout time.Duration, rateLimit float64) *Client {
	...
	cacheDir := filepath.Join(homeDir, ".cache", "ga4-pp-cli")
...
func (c *Client) authHeader() (string, error) {
	...
	// 1. Explicit env-var token wins (e.g. GOOGLE_ANALYTICS_DATA_OAUTH2C set
	//    by an outer agent that already minted a token). Surface as Bearer.
	if h := c.Config.AuthHeader(); h != "" && c.Config.AuthHeaderVal == "" && c.Config.GoogleAnalyticsDataOauth2c != "" {
		return h, nil
	}
	// 2. Application Default Credentials (service-account JSON via
	//    GOOGLE_APPLICATION_CREDENTIALS, gcloud user creds, or GCE metadata).
	//    This is the canonical GA4 auth path for headless/agent use.
	if googleauth.Available() {
		tok, _, err := googleauth.Token(context.Background())
		if err != nil {
			return "", err
		}
		return "Bearer " + tok.AccessToken, nil
	}
	// 3. Fall back to any persisted OAuth2 user token.
