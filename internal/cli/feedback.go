...
func feedbackFilePath() (string, error) {
	...
	dir := filepath.Join(home, ".ga4-pp-cli")
...
func FeedbackEndpointConfigured() bool {
	return os.Getenv("GA4_FEEDBACK_ENDPOINT") != ""
}
...
func feedbackAutoSend() bool {
	v := strings.ToLower(strings.TrimSpace(os.Getenv("GA4_FEEDBACK_AUTO_SEND")))
...
func postFeedback(url string, entry FeedbackEntry) error {
	...
	req.Header.Set("User-Agent", "ga4-pp-cli/feedback")
...
func newFeedbackCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Long: `Feedback is captured locally first at ~/.ga4-pp-cli/feedback.jsonl.
When ` + "`GA4_FEEDBACK_ENDPOINT`" + ` is set and either --send is
passed or ` + "`GA4_FEEDBACK_AUTO_SEND=true`" + `, the entry is
...
func newFeedbackListCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: `  ga4-pp-cli feedback list
  ga4-pp-cli feedback list --limit 5
  ga4-pp-cli feedback list --json`,
